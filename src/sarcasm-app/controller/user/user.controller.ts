...
import { AddSarcasmErrorResponse, AddSarcasmRequestBody, addSarcasmSchema, AddSarcasmSuccessResponse } from "./user.controller.types.js"
...
export default class UserController {
    ...
    private addSarcasticCommentResponse = ({ success, requestBody, similarSarcasms }: {
        success: boolean;
    ...
    addSarcasticComment = async (req: Request, res: Response) => {
        ...
        const sarcasms: string[] = allSarcasms.map(sarcasm => sarcasm.sarcasm as unknown as string);

        const { hasSimilarSarcasms, similarSarcasms } = this.sarcasmSimilarityChecker.checkSimilarSarcasms(sarcasm, sarcasms);

        if (hasSimilarSarcasms) {
            res.status(STATUS_CODES.BAD_REQUEST).json(this.addSarcasticCommentResponse({
                success: false,
        ...
        res.status(STATUS_CODES.CREATED).json(this.addSarcasticCommentResponse({
            success: true,
            requestBody: req.body,
            similarSarcasms
        }));
